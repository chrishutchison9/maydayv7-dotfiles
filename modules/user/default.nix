...
in
{
  ...
  options = {
    ...
    home-manager.users = mkOption {
      type = types.attrsOf (
        types.submoduleWith {
          modules =
            if (all (value: value.minimal) (attrValues settings)) then
            ...
            submodule (
              ...
              }:
              {
                ...
                config = {
                  ...
                  forwarded = filterAttrs (name: _: !(options ? "${name}") || name == "extraGroups") config;
                };
              }
            )
          );
  ...
  config = {
    ...
    services.displayManager.autoLogin =
      let
        find = findSingle (value: value.autologin || value.minimal) "0" "1" (attrValues settings);
      in
      rec {
        enable =
          if (find == "0") then
            false
          else if (config.services.displayManager.defaultSession == null) then
            throw "Graphical Environment must be present for Automatic Log-In"
          else if (find == "1") then
        ...
        user = mkIf enable (
          findFirst (name: with settings."${name}"; autologin || minimal) "nixos" (attrNames settings)
        );
      };
